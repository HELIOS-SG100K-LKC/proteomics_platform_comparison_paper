#Correlation SomaLogic pre-ANML
...
ggplot(data = Data, aes(x = Class, y = Mean_Cor_Tech, fill = Class, color = Class)) +
  geom_violin(trim = TRUE, linewidth = 0.8, color = "#0072B5FF", fill = "#0072B5FF") +
  ...
  ggtitle("SomaLogic pre-ANML Sample-Level")
rm(Class)
rm(Data)
...
#Stratify by LoD
LOD_SomaLogic <- read.csv("SomaLogic_pre-ANML_LoD/SomaLogic_LOD.csv")
...
Data$Normalization <- rep("pre-ANML", 300)
write.csv(Data, "Correlation_Set_pre-ANML_Sample.csv", row.names = F)
...
Data$Normalization <- rep("pre-ANML", 300)
write.csv(Data, "Correlation_Dilution_pre-ANML_Sample.csv", row.names = F)
...
ggplot(data = Data, aes(x = Class, y = Mean_Cor)) +
  ...
  theme(plot.title = element_text(size = 15, face = "bold")) +
  theme(plot.title = element_text(hjust = 0.1)) +
  ggtitle("SomaLogic pre-ANML Protein-Level") +
  labs(y = "Mean Pearson correlation between technical replicates", x = "Violin plot")
rm(Class)
rm(Data)
rm(Mean_Cor_SomaLogic_Tech)
...
Data$Normalization <- rep("pre-ANML", 10675)
write.csv(Data, "Correlation_Set_pre-ANML_Protein.csv", row.names = F)
...
Data$Normalization <- rep("pre-ANML", 10675)
write.csv(Data, "Correlation_Dilution_pre-ANML_Protein.csv", row.names = F)
