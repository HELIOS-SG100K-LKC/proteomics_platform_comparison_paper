...
ggplot(Data, aes(x = Threshold, y = Rate)) +
  geom_bar(stat = "identity", position = "dodge", color = "#0072B5FF", fill = "#0072B5FF") +
  ...
  theme_classic() +
  ggtitle(paste0("SomaLogic pre-ANML")) +
...
Data$Normalization <- rep("pre-ANML", 1500)
write.csv(Data, "Sample_Missingness_pre-ANML_Sets.csv", row.names = F)

rm(Class)
rm(Data)

#Protein
Data <- data.frame(Rate = c(0.01821862,0.008377425,0.01528699), Set = c("Set 1","Set 2","Set 3"))
...
Data$Normalization <- rep("pre-ANML", 3)
write.csv(Data, "Protein_Missingness_pre-ANML_Sets.csv", row.names = F)
...
Data$Normalization <- rep("pre-ANML", 1500)
write.csv(Data, "Sample_Missingness_pre-ANML_Dilution.csv", row.names = F)
...
#Protein
Data <- data.frame(Rate = c(0.01819454,0.002116402,0.009478673), Set = c("1:5","1:200","1:20000"))
ggplot(Data, aes(x = Set, y = Rate, fill = Set, color = Set)) +
  geom_bar(stat = "identity", position = "dodge") +
  theme_classic() +
  scale_fill_manual(values = c("1:5" = "#0072B5FF", "1:200" = "#E18727FF", "1:20000" = "#20854EFF")) +
...
Data$Normalization <- rep("pre-ANML", 3)
write.csv(Data, "Protein_Missingness_pre-ANML_Dilution.csv", row.names = F)
