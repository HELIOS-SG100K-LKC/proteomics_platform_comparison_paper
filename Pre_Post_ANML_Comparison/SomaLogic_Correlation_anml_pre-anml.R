...
#Analyte Annotation
Analyte_Annotation2 <- read.csv("SomaLogic_pre-anml_Preprocessing/Somalogic_Analyte_Annotation_All.csv")
...
Protein_Cor <- vector()
for (i in 1:10675) {
  Protein_Cor[i] <- cor(ProtMatrix[,i], ProtMatrix2[,i], method = "spearman")
}
rm(i)
summary(Protein_Cor)
...
ggplot(data, aes(x = correlation, fill = group)) +
  ...
  labs(title = "SomaLogic Protein-level", x = "Spearman correlation between pre-ANML and ANML", y = "Frequency", fill = "Correlation Range") +
...
write.csv(data.frame(Protein = colnames(ProtMatrix), Corr = Protein_Cor), "Protein_Level_Spearman_Correlation_anml_pre-anml.csv", row.names = FALSE)
